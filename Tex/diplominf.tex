...
durchgeführt wird.
Anschließend werden zwei Metriken, PSNR und SSIM zum Vergleich von Bildern vorgestellt,
...
Die Tiefenbilder wurden mit 16 Bit quantisiert und die Farbbilder mit 24 Bit, dabei entfallen jeweils
...
Für die Projektion werden die Position der Clippingebenen durch die Werte $z_{near}$ und $z_{far}$ beschrieben
und es wird der Öffnungswinkel der Kamera benötigt.

Von beiden Szenen wurden 493 Bilder erzeugt.
Beide Datensätze lassen sich in Abhängigkeit der Winkelschrittweite in 3 Sequenzen unterteilen.
Eine Auflistung zeigt die Tabelle \ref{tab:datasets}.
...
Wobei \textit{TestSpheres} ein synthetisch erzeugter Satz ist, der aus wenigen Partikeln geniert wurde.
...
Dazu wird aus jedem Pixel $d \in D$ ein Vertex erzeugt und mit Hilfe der Rückprojektion,
lassen sich die Vertices in die gewünschten Positionen transformieren.
...
Mit Hilfe der Modellmatrix $M = RT$, die aus einer Rotation und einer Translationsmatrix besteht, 
...
Schließlich kann die Projektion aus dem Kamerakoordinatensystem in normalisierte Gerätekoordinaten wird mit Hilfe
...
$-1$ bis $1$. 
Diese lassen sich in Bildschirmkoodrinaten umrechnen, indem die drei Komponenten in das
Intervall $[0, 1]$ übersetzt werden und anschließend werden die x- und die y-Komponenten
...
Jede Transformatiosmatrix $T_i$ setzt dabei aus einer Modellmatrix $M_i$, einer Kameramatrix $V_i$ und einer
...
Zunächst $v_i$ liegt in Bildschirmkoordinaten des Pixels $d_i$ vor und muss in normalisierte Gerätekoordinaten übersetzt werden. 
...
in das Intervall $[-1, 1]$ transformieren werden.
Jetzt liegt $v_i$ in normalisierten Gerätekoordinaten vor.
Durch die Multiplikation von der inversen Transformationsmatrix $T_i^{-1}$ mit $v_i$, kann der Vertex in
seine Modellkoordinaten überführt werden und im Anschluss kann dieser mit Hilfe von $T_j$, 
...
und dieses zu Bildsynthese zu verwenden.
...
Die Anzahl der Vertices ist abhängig von der Auflösung $w \times h$ 
und lässt sich folgendermaßen berechnen: $3 \cdot w \cdot h$.
Die x- und y-Komponenten lassen sich aus der bekannten Auflösung vorausberechnen und
müssen im Falle eine Auflösungsänderung neu bestimmt werden.
...
Problematisch bei diesem Verfahren ist, das die Anzahl der Dreiecke von der Auflösung
abhängt und der Rechenaufwand der Klientseitigen Bildsynthese quadratisch mit der Auflösung steigt.

\section{Delaunay-Triangulierung}

Die zweite Variante die in dieser Arbeit untersucht wird, ist die eine adaptive Vernetzung des Tiefenbildes.
Diese wird vom Server durchgeführt und das erzeugte Dreiecksnetz wird anschließend als solches zum Klient gesendet.
Das Verfahren lässt sich in drei Teile unterteilen. 
Zuerst muss aus dem Tiefenbild $D$ eine Menge von Punkten $P \subset D$ definiert werden.
Anschließend werden die Punkte $p \in P$ zu Dreiecken verbunden.
Im letzten Schritt kann das entstandene Netz weiter optimiert werden. 

Entscheidend für das Ergebnisnetz ist die Wahl der Punkte $P$ im ersten Schritt.
Sie müssen so gewählt werden, das die Werteverläufe der Tiefenwerte möglichst
optimal wiedergegeben werden.
Das bedeutet, das insbesondere im Bereich von Kanten Punkte gesetzt werden müssen,
aber auch dort, wo Flächen nicht planar sind, damit die Oberflächenverläufe erhalten bleiben.
Um diese Aufgaben zu lösen, wurden die beiden Verfahren von Banno \textit{et al.} \cite{Banno:2012:RCD:2407516.2407579}
und Lee \textit{et al.} \cite{900943} implementiert.

\subsubsection{Besondere Punkte}

Um die Menge $P$ zu erzeugen, werden die Gradienten des Tiefenbildes $\nabla_x$ und $\nabla_y$ 
mit Hilfe des Sobel-Operators berechnet. 
