...
Sie definieren \textit{THIN}-Systeme, als eine Klasse von Remote-Visualisierungssysteme,
...
Das Ziel von \textit{THIN}-Systemen von besteht im darin Desktop-Anwendungen fern steuern zu können.
...
Zu diesem Zweck konstruieren mit Hilfe des Alpha-Farbkanals der
...
Weitere geometrische Kompressionsverfahren für Vertices, Indizes und Normalen, sind in der Arbeit von
...
gibt das Verhältnis zwischen dem Maximalwert des Signals, auch als Nutzleistung bezeichnet und dem Einfluss des Rauschens, der Störleistung an. 
...
Dieser lässt sich bestimmen, in dem innerhalb eines Fenster der Größe $m \times n$
...
Im Gegensatz, zum PSNR, ist die von Wang \textit{et al.} entwickelt Metrik
\textit{structural similarity index} auf ein Modell menschlicher Wahrnehmung angepasst \cite{Wang04imagequality} ist.
...
Die mittlere Intensität, es diskreten Signals $x$ lässt sich in
...
misst wird durch dem Term $l(x, y)$ bezeichnet.
...
Der Kontrast wird mit Hilfe der Standardabweichung $\sigma_{x}$ approximiert.
...
indem es durch seine eigene Standartabweichung geteilt wird und 
...
Der $SSIM(x, y)$ misst die Güte jedoch nur lokal, um
eine Aussage über das gesamte Bild treffen zu können, wird der
...
Der Parameter $\gamma \in [0,1]$ wir genutzt, um die Ausprägung von schwachen 
...
Bei dem Floyd-Steinberg Algorithmus handelt es sich, um einen einfachen und effizienten Dithering-Algorithmus, der auf einen Fehler-Diffusionsverfahren
fußt.
Das Merkmalsbild wird mit Hilfe des Schwellwerts $\delta$ binärisiert.
...
Zudem wird der Binärisierung resultierende Quantisierungsfehler, des Pixels $d \in D$, entsprechend der Abbildung \ref{fig:FloydSteinberg_Quantisierungsfehler} auf die benachbarten Pixel verteilt. 
...
Dadurch lässt sich Verteilung von dem Quantisierungsfehler, ohne einen zusätzlichen Puffer, in einem einzigen Durchlauf berechnen.
...
dann wird die Menge $P$ mit der Position, von dem gerade bearbeiteten Pixel erweitert.
...
Die Abbildung \ref{fig:refined_mesh_1} zeigt ein nachbearbeitetes Dreiecksnetz.
...
Im ersten, werden Details über die Kommunikation zwischen Klient und Server behandelt.
...
und der Server wartet darauf, dass sich ein Klient verbindet.

Die Kommunikation zwischen Server- und die Klient-Komponente basiert auf dem
...
Möchte der Klient die Auflösung ändern schickt er die Anfrage, dem Beispiel \ref{lst:jsonrpc_request_resize}
...
Der Server stellt eine Menge von Methoden zur Verfügung, die durch den Klient initiiert,
...
\begin{figure}[H]
	\begin{tikzpicture}
		...
		\node[Komponent] (CLIENT) at (10, 0) {:Klient};
...
Kann sich der Klient erfolgreich mit dem Server verbinden, fragt er die \textit{Playlist} ab.
Diese liefert dem Klient alle Informationen über die zur Verfügung stehenden Szenen.
Sie enthält für jeden Frame einer Szene, dessen Kamerainformationen.
Nachdem der Klient alle Szenen kennt, kann er den Server mitteilen, welche dieser laden soll.
...
Wird der Frame empfangen, kann dieser vom Klient anschließend extrapoliert werden.
...
Der x- und y-Anteil des Gradienten, wird mit einem Schwellwert verglichen.
Ist einer dieser beiden Gradientkomponenten größer als der Schwellwert, 
...
und es werden die Resultate der getesteten Parameterkonfigurationen werden vorgestellt.
Zusätzlich werden auftretende Besonderheiten kenntlich gemacht.

Um die Vollvernetzung und die Delaunay-Triangulierung auswerten zu können stehen,
wie bereits erwähnt zwei Szenen zur Verfügung, \textit{TestSpheres} und \textit{CoolRandom}.
Von diesen wird der jeweils erste Frame als Referenz für die Extrapolation gewählt.
Aus dessen Tiefenbild, wird mit den jeweiligen Verfahren ein Dreiecksnetz erzeugt,
welches zusammen mit dem dazugehörigen Farbbild, zum Klient gesendet wird.
Der Klient rendert das Dreiecksnetz aus allen Kameraperspektiven der Szene und
sendet die Ergebnisse zurück Server.
Die Qualität der Ergebisse, wird mit dem PSNR und dem SSIM bestimmt.
Es wird gemessen, wie lange der Server zur Kodierung des ersten Frames benötigt.
Die Kompressionszeit des Farbbildes wird aufgezeichnet
und im Fall der Vollvernetzung, die Kompressionszeit des Tiefenbildes.
Für die Algorithmen die auf der Delaunay-Triangulation basieren,
wird die Erzeugungszeit des Dreiecksnetzes, in drei Teile unterteilt.
Erstens, die Erzeugung der Punktmenge $P$.
Zweitens die Vernetzung mit der Delaunay-Triangulierung.
Und drittens, die Zeit die für die Nachbearbeitung und die Dekodierung des
Netzes in das JSON-Objekt benötigt wird.
Klientseitig wurde die Zeit gemessen, die für die Extrapolation benötigt wird.

Für die Vollvernetzung wurde das Tiefenbild, mit 8 und 16 Bit kodiert. 
Es wurde ein 3 Pass und ein 5 Pass Verfahren verwendet, um die Transparenz zu realisieren.
Und schließlich wurden die Schwellwerte $1, 0.9, 0.8$ für den
Vergleich mit dem Gradienten getestet.
Die besten Ergebnisse werden mit 16 Bit und dem 5 Pass
Verfahren erzielt werden, wobei der Gradientenschwellwert 1 beträgt.
Diese Konfiguration wird als Basis zum Auswertung der Delaunay-Verfahren verwendet.
Je kleiner der Schwellwert für den Gradienten wird, umso schlechter
ist das Ergebnis bei kleinen Kamerawinkelunterschieden, während
das Ergebnis bei großen Winkelunterschieden besser wird.
Die Unterschiede zwischen den verschiedenen Konfigurationen sind marginal
und werden deshalb nicht gesondert präsentiert.  
...
Und die Bäume mit den maximalen Baumtiefen von $10, 9$ und $8$ erzeugt.
Für jede Baumtiefe wurden die Schwellwertkonfigurationen
$l \in \{0.1,0.2, ..., 1.0\}$ und $i \in \{0.1, ..., l\}$ gemessen.
Für alle $l > 0.3$ und $i > 0.3$, wurden zusätzlich die Netzverfeinerung durchgeführt. 
Mit den Schwellwerten $\alpha \in \{0.1, 1, 10\}$ und $\beta \in \{0.1, 0.2\}$.
Die Erzeugung der Punktemenge, erhoben mit dem \textit{Floyd-Steinberg}-Fehlerdiffusionsverfahren,
wird mit den Parametern $t,g \in \{0.1,0.2, ..., 1.0 \}$ evaluiert.
Bei diesem Ansatz wurde die Netzverfeinerung für Konfigurationen $t > 0.8$ und $g > 0.8$
durchgeführt.

Die folgenden Diagramme zeigen ausgewählte Beispiele von den erhobenen Daten.
In den Legenden, wird \textit{Quadtree} mit QT und \textit{FloydSteinberg} mit FS abgekürzt.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\draw[yshift=\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=blue}] (0,0) node[right]{QT, $D_{MAX} = 10$, $i,l = 0$};
		\draw[yshift=2\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=green}] (0,0) node[right]{FS, $t,g = 0.3$};
		\end{scope}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
	...
	\caption{Die Diagramme a und b zeigen die Ergebnisse des Datensatzes \textit{TestSpheres},
			 ...
			 die Ergebnisse des PSNR gezeigt werden. $l,i = 0$}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\draw (0,0) -- 
			plot[mark=*, mark options={fill=red}] (0,0) node[right]{Full};
		\draw[yshift=\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=blue}] (0,0) node[right]{QT};
		\draw[yshift=2\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=green}] (0,0) node[right]{FS};
		\end{scope}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
...
Dabei liefert die Vollvernetzung insgesamt die besten Ergebnisse, gefolgt vom \textit{Quadtree}
und das Schlusslicht bildet der \textit{FloydSteinberg} Ansatz.
Interessant ist dabei, dass die Ergebnisse des \textit{CoolRandom} Datensatzes wesentlich
schlechter ausfallen, insbesondere beim PSNR.
Beim PSNR steigen die Werte, ab einem Winkel von 70 Grad wieder leicht an.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(3,4)}] 
		\draw (0,0) -- 
			plot[mark=*, mark options={fill=red}] (0,0) node[right]{Quadtree, $D_{MAX} = 10$};
		\draw[yshift=\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=blue}] (0,0) node[right]{Quadtree, $D_{MAX} = 9$};
		\draw[yshift=2\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=green}] (0,0) node[right]{Quadtree, $D_{MAX} = 8$};
		\end{scope}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\draw (0,0) -- 
			plot[mark=*, mark options={fill=red}] (0,0) node[right]{$D_{MAX} = 10$};
		\draw[yshift=\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=blue}] (0,0) node[right]{$D_{MAX} = 9$};
		\draw[yshift=2\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=green}] (0,0) node[right]{$D_{MAX} = 8$};
		\end{scope}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
...
Die Diagramme \ref{fig:Daten_QT_1098} tragen die Ergebnisse des \textit{Quadtree}
Ansatzes ab. Dabei wird nur die Maximaltiefe des Baumes variiert.
Für den \textit{TestSphere}-Datensatz unterscheiden sich die Ergebnisse kaum.
Während beim \textit{CoolRandom}-Datensatz eine klare Tendenz zu erkennen ist,
größere Maximaltiefen führen zu besseren Ergebnissen. Der Unterschied ist bei einem Winkel
vom 20 bis 30 Grad am größten.
Auch hier steigen die Werte des PSNR ab einem Winkel von 70 Grad wieder leicht an.
...
Und die Qualitätsunterschiede, der Parameterkonfigurationen besonders deutlich werden.
...
\begin{figure}
	...
	\begin{tikzpicture} [scale=0.8]
		...
		\end{axis}
	\end{tikzpicture}}
	\caption{Ergebnisse des \textit{Quadtree}-Ansatz mit \textit{TestSpheres},
			 ...
			 Auf der rechten Seite wird die Gesamtkompressionszeit des Netzes abgetragen.}
	\label{fig:qt_}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Ergebnisse des \textit{Quadtree}-Ansatz mit \textit{CoolRandom},
			 ...
			 zeigen die Güte mit Hilfe des SSIM Wertes. 
			 Auf der rechten Seite wird die Gesamtkompressionszeit des Netzes abgetragen.}
	\label{fig:Daten_QT_1098_Cool}
\end{figure}

In der Abbildung \ref{fig:Daten_QT_1098_Cool} sind die Ergebnisse, von dem \textit{TestSphere} zu sehen.
Dabei Fällt auf, dass bei einem Festen Winkel von 20 Grad, die Ergebnisse, hinsichtlich ihrer
Qualität und in Abhängigkeit zur Baumtiefe kaum variieren.
Bei einer Baumtiefe von 10, sind die Konstruktionszeiten für das Netz, bei einem Parameter 
von $i = 0$ rot hervorgehoben und damit zu groß für eine interaktive Visualisierung.

Die Gleichen Daten, dieses mal von der Szene \textit{CoolRandom} sind in Abbildung \ref{fig:Daten_QT_1098_Cool} zu sehen.
Für eine maximale Baumtiefe von 10, liegen die Gütewerte in einem noch akzeptablen
Bereich. 
Zu sehen ist eine Verschlechterung, bei steigendem $l$ und $i$.
Die Berechnungszeiten, sind für alle Konfigurationen zu langsam.
Bei einer maximalen Baumtiefe von 9 dagegen sind die Gütewerte bereits deutlich
geringer, als bei 10.
Dafür über alle Parameterkonfigurationen konstant verlaufen.
Mit Blick auf die Ausführungszeiten ist zu erkennen, dass diese mit abnehmenden
$l$ und $i$ sinken.
Bei einer Baumtiefe von 8 sind alle Konfigurationen schnell genug, für die 
interaktive Visualisierung.
Die Güte liegt dabei konstant für alle $l$ und $i$ unter $0.8$.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 ...
		 			 colorbar, colorbar style={xlabel={SSIM}}, colorbar style={xlabel=Dreiecke, y dir=reverse}, enlargelimits=true, 
		 			 xtick={0.0,0.1,...,1.1},
		     		 ytick={0.0,0.1,...,1.1},
		     		 xmajorgrids=true, ymajorgrids=true]
	...
	\begin{tikzpicture}[scale=0.75]
		\begin{axis}[view={0}{90}, grid=major,
		 			 ...
		 			 ylabel=$l$,
		 			 ylabel style={rotate=-90},
		 		%	 zticklabel style={/pgf/number format/sci},
		 			 %point meta min=0.7, point meta max=0.9,
		 			 colormap={greenyellow}{
		   			 	rgb255(0cm)	 =(200,200,200)
		   			% 	rgb255(0.5cm) =(255,255,255) 
...
Analog zu den Schwellwerten der \textit{Quadtree}-Methode, werden die Parameter $t$ und $g$,
vom \textit{FloydSteinberg}-Verfahren abgebildet.
...
Dabei sind die Ergebnisse \ref{fig:data_floydSteinberg}, für die Szene \textit{TestSphere} hinsichtlich der Güte, für alle Konfigurationen akzeptabel.
Bei der Bereichnungszeit zeigt sich, dass erst ab den Konfigurationen $t > 0.6$ und $g > 0.5$,
die Kompressionszeiten für eine interaktive Visualisierung klein genug sind.
Anders sieht es bei dem Datensatz \textit{CoolRandom} aus.
Hier erreicht keine Parameterkonfiguration eine ausreichend kleine Konstruktionszeit.
Die Gütewerte liegen alle unterhalb von $0.8$. Dabei zeigt sich eine Verschlechterung,
für Konfigurationen mit $t > 0.6$ und $g > 0.5$.


% % % % % % % % % % %hjasdhkjashdjkashdsajdhjkashdhj
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4pre.csv};
	%	\addlegendentry{Case 1}	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4A1.0J0.2pre.csv};
		\end{axis}
		%legend
		\begin{scope}[shift={(3,4)}] 
		\draw (0,0) -- 
			plot[mark=*, mark options={fill=green}] (0,0) node[right]{$\alpha = 0.1$, $\beta = 0.2$};
		\draw[yshift=\baselineskip] (0,0) -- 
			plot[mark=*, mark options={fill=blue}] (0,0) node[right]{$\alpha = 0.1$, $\beta = 0.1$};
		\draw[yshift=2\baselineskip] (0,0) -- 
	...
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D10L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4A1.0J0.2pre.csv};
	...
	\begin{tikzpicture}
	 ...
	 every axis x label/.style={at={(ticklabel* cs:1.05)}, anchor=south}]
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D9L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
	\subfigure[]{	
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4A1.0J0.2pre.csv};
	...
	\begin{tikzpicture}
	 ...
	 every axis x label/.style={at={(ticklabel* cs:1.05)}, anchor=south}]
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_7_512x512D8L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
...
der Güte auswirkt.
Zu diesem Zweck wurde für die Szenen eine Parameterkonfiguration exemplarisch
ausgewählt um den Sachverhalt zu verdeutlichen.
Dabei ist zu sehen, dass die optimierten Netze bei kleinen Winkeln bis zu 10 Grad
schlechtere Ergebnisse liefert, als die nicht optimierten Netze.
Bei größeren Winkeln jedoch bessere.
Weiterhin lässt sich erkennen, das die Netzoptimierung insbesondere für kleine
Maximaltiefen des Baums bessere Ergebnisse liefert, als für große.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[axis lines = middle, enlargelimits = true, ylabel={SSIM}, xlabel={in Grad}, xtick={0,5,10,20,...,90},
			 every axis y label/.style={at={(ticklabel* cs:1.05)}, anchor=east},
			 every axis x label/.style={at={(ticklabel* cs:1.05)}, anchor=south}]		
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4A1.0J0.2pre.csv};
	...
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D10L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
	\subfigure[]{
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4A1.0J0.2pre.csv};
	...
	\begin{tikzpicture}
	 ...
	 every axis x label/.style={at={(ticklabel* cs:1.05)}, anchor=south}]
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D9L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
	\subfigure[]{	
	\begin{tikzpicture}
		...
		\addplot[red, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=m, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4A1.0J0.2pre.csv};
	...
	\begin{tikzpicture}
	 ...
	 every axis x label/.style={at={(ticklabel* cs:1.05)}, anchor=south}]
		...
		\addplot[red, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4pre.csv};	 
		\addplot[blue, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4A0.1J0.1pre.csv};
		\addplot[green, only marks, mark=x] table [x=a, y=p, col sep=comma] {div_data_mean_6_512x512D8L0.6I0.4A0.1J0.2pre.csv};
		\end{axis}
	\end{tikzpicture}}
...
Das Ziel bestand darin, ein Verfahren zu entwickeln, das den Werteverlauf von Tiefenbildern, mit Hilfe
von Dreiecksnetzen approximiert und die Güte, in Abhängigkeit des Kamerawinkelunterschieds zum
Originalbild, zu evaluieren.
Zu diesem Zweck wurden mit einem \textit{Quadtree} beziehungsweise der Floyd-Steinberg Methode
eine Menge von Punkten bestimmt. 
Diese wird mit Hilfe der Delaunay-Triangulierung zu einem Dreiecksnetz verknüpft.
In einen weiteren Schritt, kann die Triangulierung optimiert werden.

Die besten Resultate, der verschieden Varianten liefern die in der Abbildung \ref{fig:Daten_best_cases} 
abgebildeten Parameterkonfigurationen. Wobei diese auch die meisten Dreiecke produzieren.
Es zeigt sich sehr deutlich, das die Güte der Ergebnisse von der Szene und ihrem Inhalt abhängt.
