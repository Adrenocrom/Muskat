...
Das Ziel von \textit{THIN}-Systemen besteht, im darin Desktop-Anwendungen fernsteuern zu können.
...
Die Bäume werden mit den maximalen Baumtiefen $d_{max} \in \{10, 9, 8\}$ erzeugt.
Für jede Baumtiefe $d_{max}$ wurden die Schwellwertkonfigurationen
...
Die Gütewerte für die Baumtiefe $d_{max} = 8$, liegen dabei deutlich unterhalb der anderen.
...
\begin{figure}
	\centering
	\subfigure[\textit{TestSpheres}, $d_{max} = 10$]{
	...
	\subfigure[\textit{TestSpheres}, $d_{max} = 9$]{
	\begin{tikzpicture} [scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 xlabel=$i$,
		 			 ylabel=$l$,
	...
	\subfigure[\textit{TestSpheres}, $d_{max} = 8$]{
	\begin{tikzpicture} [scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 xlabel=$i$,
		 			 ylabel=$l$,
...
Es ist zu erkennen, dass die Güte mit der maximalen Baumtiefe $d_{max}$ abnimmt.
...
Bei $d_{max} = 10$ und für $i = 0$ ist die Konstruktionszeit für alle $l$ größer
als 120 ms. 
Für die maximale Baumtiefe $d_{max} = 9$ überschreitet die Konfiguration $l,i = 0$ die Marke
von 120 ms.

\begin{figure}[H]
	\centering
	\subfigure[\textit{CoolRandom}, $d_{max} = 10$]{
	\begin{tikzpicture}[scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 xlabel=$i$,
		 			 ylabel=$l$,
	...
	\subfigure[\textit{CoolRandom}, $d_{max} = 9$]{
	\begin{tikzpicture} [scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 xlabel=$i$,
		 			 ylabel=$l$,
	...
	\subfigure[\textit{CoolRandom}, $d_{max} = 8$]{
	\begin{tikzpicture} [scale=0.8]
		\begin{axis}[view={0}{90}, grid=major,
		 			 xlabel=$i$,
		 			 ylabel=$l$,
...
Baumtiefe $d_{max} = 10$ beträgt.
Für die anderen beiden maximalen Baumtiefen $8$ und $9$, ist die Güte für alle $i$ und $l$
relativ konstant.
Auch bei dieser Szene zeigt sich das bei $d_{max} = 10$ die Konstruktionszeiten am größten sind.
...
\begin{figure}[H]
	...
	\caption{Die Diagramme zeigen für den \textit{Quadtree}-Ansatz,
			 ...
			 in Abhängigkeit der Parameter $l,i$ und $d_{max}$.
			 Dabei liegt die maximale Baumtiefe in a,b bei 10,
...
In der Abbildung \ref{fig:Daten_QT_1098_Tri} ist zu sehen, dass die Anzahl der Dreiecke
mit der kleiner werdender Baumtiefe sinkt.
Die Dreiecksanzahl von Netzkonfigurationen des \textit{TestSpheres}-Datensatzes 
ist dabei für $i = 0$ größer als für alle anderen.
Am größten ist diese, wenn auch $l = 0$ ist.
Ein Ähnliches Bild ergibt sich für den \textit{CoolRandom}-Datensatz.
Hier ist der Unterschied zwischen den verschieden $l$ und $i$ gleichmäßiger.
Wenn sowohl $l$, als auch $i$ kleiner werden, steigt die Anzahl der Dreiecke.
...
vom \textit{FloydSteinberg}-Verfahren abgebildet \ref{fig:data_floydSteinberg}.
Dabei sind die Ergebnisse \ref{fig:data_floydSteinberg}, für die Szene \textit{TestSpheres} hinsichtlich der Güte, für alle Konfigurationen sehr gut.
Bei der Bereichnungszeit zeigt sich, dass erst ab den Konfigurationen $\delta > 0.6$ und $\gamma > 0.5$,
die Konstruktionszeiten für eine interaktive Visualisierung klein genug sind.
Die Anzahl der Dreiecke ist bei kleinen Werten für $\gamma$ oder für $\delta$ am Höchsten.
...
mit $\delta > 0.6$ und $\gamma > 0.5$.
Die Konstruktionszeiten des Netzes sind für alle Parameterkonfigurationen zu groß. 
Mit der Anzahl der Dreiecke ist dabei ähnlich verteilt wie für die Szene \textit{TestSpheres}.
...
Deutlich zu sehen ist, dass beim SSIM und bei dem PSNR, dass bei kleinen 
Winkeln das nicht nachbearbeitete Netz besser ist als die nachbearbeiteten.
Dabei ist zu sehen, das für jede Konfiguration ein Kipppunkt existiert, 
bei dem das optimierte Netz bessere Ergebnisse liefert.
Dabei 
