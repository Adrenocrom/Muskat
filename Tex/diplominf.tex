...
\acknowledgments{An dieser Stelle möchte ich meinen Eltern, meinen Freunden und meinen Betreuer für ihre Unterstützung bedanken.}

\abstracten{
Remote-visualization is a technique, which seperates rendering and displaying of images
into two components.
In this paper, such a system was developed, based on the WebSocket-protokol.
In order to hide the latency, as a result of the communication between both components, from the user,
it is possible to use additional geometric information to extrapolate the image on the client side.
The depth image, produced in the rendering step, are used as the necessary
geometric inforamtion.
Triangle-meshes can be used to approximate the depth image to transfer the information.
Two methods were compared, both use the Delaunay-triangulation.
One produces the point set needed for meshing with Floyd?Steinberg dithering
and the second one with a quadtree.
...
Optical quality is messured by SSIM and PSNR and the mesh complexity is equal to number of triangles.
Result of the comparison is, that the best method with respect to all critera is the quadtree.
}
...
Die zur Vernetzung benötigte Punktmenge wird zum einem mit dem \textit{FloydSteinberg}-Fehlerdiffusionsverfahren und zum anderen durch ein \textit{Quadtree}-Ansatz generiert.
...
die Netzkomplexität sowie der Rechenzeit. 
...
Ein Quadtree ist eine Baumdatenstruktur, in der jeder Knoten maximal vier Kindknoten hat.
...
Im Speziellen geht es dabei um Nachrichten, die dazu dienen, dass der Kommunikationsteilnehmer bestimmte 
Methoden mit bestimmten Parameterkonfigurationen startet.
...
\begin{figure}[H]
	...
	\caption{Die Diagramme a, b stellen die Ergebnisse von dem \textit{CoolRandom} Datensatz
			 und c, d die des \textit{TestSpheres}-Datensatzes, erhoben mit \textit{FloydSteinberg}-Ansatz dar.
...
Insgesamt liefert der \textit{Quadtree} weitaus bessere Resultate als die FloydSteinberg-Methode.
...
Die Abbildungen \ref{fig:nettings} und \ref{fig:floyd_steinberg_nett}
...
\begin{figure}
	...
	\caption{Die Dreiecksnetze a-c wurden mit dem FloydSteinberg-Ansatz erzeugt und die Dreicksnetze d-i mit dem \textit{Quadtree}-Ansatz.}
\end{figure}
...
entsteht bei größeren Kamerawinkelunterschieden zwangsläufig ein Fehler, wie in Abbildung \ref{fig:beleuchtung_fehler}.
...
\begin{figure}
	\centering	
	\subfigure[$t_{max} = 6$]{\includegraphics[width=.49\textwidth]{img/testspheres_00189.png}}
	\subfigure[$t_{max} = 8$]{\includegraphics[width=.49\textwidth]{img/frameFULL_00490.png}}
	\caption{title}
...
welches auf dem FloydSteinberg-Algorithmus basiert, und ein \textit{Quadtree}-Ansatz
implementiert.
...
könnte in Verbindung mit verschiedenen Detailstufen 
