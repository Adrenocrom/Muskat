...
\acknowledgments{An dieser Stelle möchte ich meiner Familie, meinen Freunden und meinen Betreuer für ihre Unterstützung bedanken.}

\abstracten{
Remote-Visualization is a technique to seperate rendering and the display of images
into two distinct components.
In this thesis, this technique has been implemented as a system based on the WebSocket protocol.

In order to hide the latency, as a result of the communication between both components, from the user,
it is possible to use additional geometric information to extrapolate the image on the client's side.
The depth image, produced during the rendering step, is used as the required
geometric data.
Triangle meshes can be used to approximate the depth image to transfer the mentioned data.
Two methods were compared - both use the Delaunay Triangulation.
One produces the point set needed for meshing using \textit{FloydSteinberg}-dithering
and the second method uses a quadtree.
Critera for the evaluation are the optical quality, which depends on the camera angle differences,
mesh complexity and construction time.
Optical quality is messured by SSIM and PSNR, and the mesh complexity is equal to the number of triangles.
As a result, the Quadtree seems to be the more favorable method regarding the mentioned criteria.
}

\abstractde{
Remote-Visualisierung ist eine Technik, welche die Bildsynthese und die Darstellung der Bilder
auf zwei Komponenten, Server und Client, aufteilt.
In dieser Arbeit wurde ein solches System auf Basis des WebSocket-Protokolls entwickelt.
Um die Latenzzeit der Kommunikation beider Komponenten vor dem Nutzer verbergen zu können,
kann clientseitig mit Hilfe zusätzlicher geometrischer Informationen eine Bildextrapolation durchgeführt werden. 
Dazu kann das bei Bildsynthese entstehende Tiefenbild genutzt werden.
Dessen Tiefeninformationen werden, im Rahmen dieser Arbeit, für die Übertragung durch ein Dreiecksnetz approximiert.
Es werden zwei Verfahren verglichen, die auf der Delaunay-Triangulierung basieren.
Die zur Vernetzung benötigte Punktmenge wird zum einem mit dem \textit{FloydSteinberg}-Fehlerdiffusionsverfahren und zum anderen durch einen \textit{Quadtree}-Ansatz generiert.
Kriterien für den Vergleich sind neben der optischen Qualität der extrapolierten Bilder, in Abhängigkeit des Kamerawinkels,
die Netzkomplexität sowie die Rechenzeit. 
Die optische Qualität wird mit dem SSIM und dem PSNR gemessen und die Netzkomplexität entspricht dabei der Anzahl der Dreiecke im Netz. 
Es hat sich gezeigt, dass der \textit{Quadtree}, im Bezug auf alle Kriterien, der deutlich bessere Ansatz ist.
}
...
Es wird gezeigt, wie die Rückprojektion und 
die Gütemetriken funktionieren.
...
Das zentrale Problem ihrer Arbeit ist die Latenz und die effiziente Übertragung der Daten vom Server zum Client.
...
Eine allgemeinere Form der Remote-Visualisierung ist die Aufteilung der Bildsynthese
...
Bei größeren Auflösungen werden Zeicheninstruktionen gesendet, die Bilder werden
...
Die bisher vorgestellten Ansätze werden in dieser Arbeit nicht aufgegriffen; sie sollen lediglich
...
Zur Erzeugung dieser Punkte kommt ein \textit{Quadtree} zum Einsatz.
...
erzeugt werden, wie zum Beispiel in der Arbeit von Chai \textit{et al.} \cite{1024050},
...
von diesen beschäftigt, wurde von Stefan Gumhold und Wolfgang Straßer geschrieben \cite{Gumhold:1998:RTC:280814.280836}.
...
Weiterhin zu beachten ist, dass das Tiefenbild des ersten Frames, aus dem \textit{TestSpheres}-Datensatz einen Fehler aufweist.
...
Was dazu führt, dass diese Kugel bei der Extrapolation nicht richtig rekonstruiert werden kann.
...
Die Metrik basiert auf der Idee, dass die Struktur der abgebildeten Objekte
...
Die zu übertragende Informationsmenge kann durch Valenz-basierte Kodierung weiter verringern werden.
...
basiert auf einer \textit{Quadtree}-Datenstruktur.
Ein \textit{Quadtree} ist eine Baumdatenstruktur, in der jeder Knoten maximal vier Kindknoten hat.
...
Die maximale Tiefe des \textit{Quadtrees} ist dabei invers proportional zur Breite der Blattregionen.
...
mit Hilfe der Differenz zwischen dem maximalen und dem minimalen Wert der Gradienten innerhalb von $R$ bestimmen:
...
in Abhängigkeit, ob es sich um einen inneren Knoten oder einen Blattknoten handelt.
Der Schwellwert für die Blattknoten wird mit $l$ bezeichnet und der für die inneren Knoten mit $i$.
Blattknoten sollten vorrangig Tiefenuntersprünge, also Kanten detektieren und somit die Kontur der dargestellten Objekte
kennzeichnen.
...
Regionen zusammensetzt, in denen die Gradientdifferenzen den entsprechenden Schwellwert überschreiten.
...
\begin{figure}
	...
	\caption{Szene \textit{TestSpheres}, $t_{max} = 9, l = 5, i = 4$. Punkte die aus 
	inneren Knoten erzeugt werden sind grün und jene, die aus den Blattknoten generiert werden, sind blau markiert. Während in a alle Punkte angezeigt werden, sind in b nur die Punkte sichtbar
	bei denen die Tiefenwerte nicht dem maximalen Tiefenwert entsprechen.}
\end{figure}
...
Um aus dem Merkmalsbild $\sigma$ die Punktmenge $P$ zu erzeugen, kommt der \textit{FloydSteinberg}-Algorithmus zum Einsatz. 
Bei dem \textit{FloydSteinberg}-Algorithmus handelt es sich um einen einfachen und effizienten Dithering-Algorithmus,
...
In dieser Arbeit wird zu diesem Zweck die Delaunay-Triangulierung aus dem opencv-Framework \cite{opencvframework} verwendet.

Bei der Vernetzung Punkte entstehen Dreiecke an den Rändern, die Tiefensprünge überspannen.
Diese können in einen weiteren Schritt verworfen werden.

\subsection{Netzoptimierung}

Die auf diese Weise entstandene Netze können in Abhängigkeit der Eingabedaten zwei Arten von Artefakten enthalten.
...
sich dem Lot der Bildebene nähert, während gleichzeitig eine bestimmte Länge in der xy-Ebene überschritten wird.
...
Um Rechenzeit zu sparen, wird empfohlen, nur den Mittelpunkt $p_m$ zwischen
...
In dem Fall, dass alle drei Kanten nicht valide sind, werden sechs neue Vertices eingefügt.
...
Es muss beachtet werden, dass jedes neu erzeugte Dreieck auf Kollinearität zu überprüfen
...
Es ist zu erkennen, dass mit größer werden $\alpha$ der Einfluss auf gekrümmte Oberflächen abnimmt.
...
Die clientseitige Extrapolation der Bilddaten wird mit Hilfe von WebGL durchgeführt.
...
Dieses Kapitel wird dabei in zwei Hauptbereiche unterteilt.
...
aus dem QT-Framework \cite{qtframework} zurückgegriffen.
...
Im Anschluss daran wird die Standardparameterkonfiguration geladen,
...
Im Speziellen geht es dabei um Nachrichten, die dazu dienen, Methoden mit bestimmten Parameterkonfigurationen auf dem Gerät des anderen Kommunikationsteilnehmers aufzurufen.
...
Diese Form der Anfragen kann zur Synchronisierung der Nachrichtenübertragung genutzt werden. 
Im Gegensatz dazu entfällt bei einer \textit{Notification} die $id$, weil keine Antwort verlangt wird.
Der \textit{Batch Request} fasst eine Menge von \textit{Requests} und \textit{Notifications} in
einer einzigen Anfrage zusammen.

Die in dieser Arbeit implementierte Variante von JSON-RPC 2.0 
...
Eine JSON-RPC-Antwort ist ebenfalls ein JSON-Objekt, das aus drei Teilen besteht.
...
Der Server stellt eine Menge von Methoden zur Verfügung, die durch den Client initiiert und 
...
genauer betrachtet.
Begonnen wird hier mit den Farbbildern.
...
Um aus $d_{up}$ und $d_{low}$ die 16 Bit-Zahl $d$ zu erhalten, genügt es, den Wert von $d_{up}$
...
Beim Zugriff auf die Farbwerte einer Textur im Shader werden diese jedoch normiert.
...
und dadurch kann die Gleichung \ref{eq:reconstruct2} weiter vereinfacht werden:
...
deren Eckpunkte in der Punktmenge $P$ auftauchen können.
...
Mit dem Effekt, dass dadurch viele kleine Dreiecke entstehen, 
dass die grün markierten Punkte sich aus mindestens zwei Eckpunkten zusammensetzen, 
...
Anschließend wird das Dreiecksnetz, texturiert mit dem Farbbild, 
...
\textit{Quadtree} und \textit{FloydSteinberg} unterschieden.
...
Für alle $l > 0.6$ und $i = 0.6$ wurde zusätzlich die Netzverfeinerung durchgeführt. 
Mit den Schwellwerten $\alpha \in \{0.1, 1, 10, 100, 1000\}$ und $\beta \in \{0.1, 0.2, ..., 1.0\}$.
Die Erzeugung der Punktemenge, erhoben mit dem \textit{FloydSteinberg}-Fehlerdiffusionsverfahren,
...
dicht gefolgt vom \textit{Quadtree}-Ansatz.
Die schlechtesten Ergebnisse liefert der \textit{FloydSteinberg}-Ansatz.
...
zwischen dem \textit{FloydSteinberg}-Ansatz und den anderen beiden Methoden. 
...
zu erkennen ist, eine höhere Streuung der Werte zu erwarten ist.
...
Anders sieht es mit der Konstruktionszeit aus; hier zeigen sich deutliche Unterschiede.
...
Je kleiner $l$ und $i$ ist, umso größer ist die Berechnungszeit.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=0.8]
		...
		\end{axis}
	\end{tikzpicture}}
	\caption{Die Diagramme stellen die Ergebnisse von dem \textit{CoolRandom}-Datensatz
			 und die des \textit{TestSpheres}-Datensatzes, erhoben mit \textit{FloydSteinberg}-Ansatz dar.}
...
Mit der Anzahl der Dreiecke verhält es sich ähnlich wie bei der Szene \textit{TestSpheres}.

Der Netzoptimierungsschritt wird ausschließlich mit dem \textit{Quadtree}-Ansatz untersucht,
da dessen Ergebnisse weitaus besser sind als die der \textit{FloydSteinberg}-Methode.
...
\begin{figure}
	...
	\begin{tikzpicture} [scale=0.8]
		...
		\end{axis}
	\end{tikzpicture}}
	\caption{Abbildung vom SSIM, bei einem konstanten Winkelunterschied von 20 Grad, und der Netzkonstruktionszeit in Abhängigkeit von $\alpha$ und $\beta$, für die Szene \textit{TestSpheres}.}
	\label{fig:nnt}
\end{figure}

In der Abbildung \ref{fig:nnt} wird der Datensatz \textit{TestSpheres} betrachtet. Es zeigt sich für $t_{max} = 10$,
dass die Güte ab $\alpha > 1$ größer als $0.9$ ist.
...
\begin{figure}
	...
	\begin{tikzpicture} [scale=0.8]
		...
		\end{axis}
	\end{tikzpicture}}
	\caption{Abbildung vom SSIM, bei einem konstanten Winkelunterschied von 20 Grad, und der Netzkonstruktionszeit in Abhängigkeit von $\alpha$ und $\beta$, für die Szene \textit{CoolRandom}.}
	\label{fig:nnc}
\end{figure}
...
\begin{figure}
	...
	\begin{tikzpicture} [scale=0.8]
		...
		\end{axis}
	\end{tikzpicture}}
	\caption{In den Diagrammen wird die Anzahl der Dreiecke in Abhängigkeit von $\alpha$ und $\beta$ abgebildet.}
	\label{fig:nnd}
\end{figure}
...
In der Szene \textit{CoolRandom} zeigt sich diese Tendenz nicht ganz so stark.
Weiterhin fällt auf, dass bei kleinen Werten für $beta$ weniger Dreiecke entstehen als bei großen Werten.
Dieser Zusammenhang zeigt sich für beide Szenen.
...
Es gibt ein paar Ausreißer, die eine Zeit von 1.5 ms nicht überschreiten.

Im Folgenden werden die vorgestellten Ergebnisse diskutiert.

\chapter{Diskussion}

In diesem Kapitel werden die zuvor präsentierten Ergebnisse diskutiert.
Dabei werden in dem Vollvernetzungsansatz die Tiefeninformationen aus dem Tiefenbild unverändert und 
unkomprimiert für die Bildextrapolation genutzt.
Sie liefert, wie es zu erwarten war, die besten Ergebnisse hinsichtlich der Qualität für beide Szenen.
Zu erwähnen ist, dass die Ergebnisse in Abhängigkeit der gewählten Szene stark variieren.
Dies hängt damit zusammen, dass die Szenen unterschiedlich beschaffen sind:
...
Das führt dazu, dass die Dreiecksnetze der Szene \textit{CoolRandom} aus vielmehr Dreiecken
bestehen als in die der \textit{TestSpheres}-Szene.
Aus diesem Grund ist die Erzeugung der Dreiecksnetze für die \textit{CoolRandom} Szene mit
weit aus mehr Rechenaufwand verbunden.
Zusätzlich steigt der Fehler hinsichtlich der Qualität deutlich schneller als in der Szene \textit{TestSpheres}.
Das resultiert daraus, dass in dem \textit{CoolRandom}-Datensatz
weitaus mehr Kugeln existieren als in der \textit{TestSpheres}-Szene, die im Tiefenbild
des ersten Frames nicht abgebildet werden, aus dem die Netze erzeugt werden
und deshalb in den extrapolierten Bildern nicht existieren.

Insgesamt liefert der \textit{Quadtree} weitaus bessere Resultate als die \textit{FloydSteinberg}-Methode.
Das Problem der \textit{FloydSteinberg}-Methode besteht darin, das extrem viele Punkte vorwiegend in der Nähe von Kanten platziert werden.
Dabei gibt es einen Unterschied zwischen äußeren und inneren Kanten.
Während die äußeren Kanten die Kontur des Objektes abbilden und sehr dicht
besetzt sind, werden innere Kanten nicht ausreichend repräsentiert.
Planare Flächen bestehen ebenso aus zu vielen Dreiecken.
Das führt dazu, dass auch bei einer sehr hohen Zahl von Dreiecken die
Qualität der extrapolierten Bilder schon bei geringen Kamerawinkelunterschieden
um einiges schlechter ausfällt als bei den anderen Methoden.
Weil insgesamt zu viele Punkte erzeugt werden, wird auch die Vernetzung langsam.
Die Abbildungen \ref{fig:netting_fq} (a-c) und \ref{fig:floyd_steinberg_nett}
verdeutlichen diesen Zusammenhang.

Im Vergleich dazu schneidet der \textit{Quadtree}-Ansatz im Bezug auf Qualität,
Rechenaufwand weitaus besser ab.
Weiterhin lässt sich die Qualität des resultierenden
Dreiecknetzes mit dem Parameter $t_{max}$, adaptiv anpassen, wie in Abbildung \ref{fig:netting_fq} (d-i).
Große Werte für $t_{max}$ bedeuten in erster Linie,
dass die Bereiche stärker unterteilt werden, so dass an markanten Stellen, wie Kanten 
...
Bei kleinen Werten für $t_{max}$, werden aufgrund der großen \textit{Quadtree}-Regionen,
wie in der Abbildung \ref{fig:smalltmax} zu sehen ist, ganze Bereiche nicht
vom Dreiecksnetz repräsentiert.
Außerdem werden bei der Extrapolation einzelne Regionen deutlich sichtbar.
...
Die Güte wird durch $l$ und $i$ deshalb kaum beeinflusst, weil sowohl äußere als auch innere Kanten auch für große Werte von $l$ ausreichend abgebildet werden.
Der Parameter $i$ bestimmt vorwiegend die Unterteilung gekrümmter
Flächen; bei kleinen Winkeln wird die Güte selbst bei einer gröberen Unterteilung kaum verschlechtert.
Es sind große Werte für $l$ zu empfehlen, beispielsweise $0.7$.
Der Parameter $i$ sollte kleiner oder gleich $l$ sein. 

\begin{figure}[H]
	\centering
	\subfigure[$t_{max} = 6$]{\includegraphics[width=.49\textwidth]{../results/7/512x512_Delaunay/D6/L0.7/I0.5/_pre/frame_00000.png}}
	\subfigure[$t_{max} = 8$]{\includegraphics[width=.49\textwidth]{../results/7/512x512_Delaunay/D8/L0.7/I0.5/_pre/frame_00000.png}}

	\subfigure[$t_{max} = 6$]{\includegraphics[width=.49\textwidth]{../results/7/512x512_Delaunay/D6/L0.7/I0.5/_pre/delaunay.png}}
	\subfigure[$t_{max} = 8$]{\includegraphics[width=.49\textwidth]{../results/7/512x512_Delaunay/D8/L0.7/I0.5/_pre/delaunay.png}}
	\caption{Darstellung des ersten Frames aus \textit{TestSpheres}, mit den dazugehörigen Dreiecksnetzen. Dabei wurde $l = 0.7$ und $i = 0.5$ gewählt.}
	\label{fig:smalltmax}
\end{figure}

Die Nachbearbeitung der Netze bewirkt grundsätzlich einen Gütezuwachs
...
zu erkennen ist.
Diese Artefakte in Form von Schnittkanten sorgen dafür, dass die Güte
...
Dieser Effekt wird in der Szene \textit{CoolRandom} besonders deutlich.
Beispiele zeigt die Abbildung \ref{fig:nettings}.

\begin{figure}[H]
	\centering
	\subfigure[$\delta = 0.5, \gamma = 0.5$]{\includegraphics[width=.3\textwidth]{img/delaunay_T5G5.png}}
	\subfigure[$\delta = 0.5, \gamma = 1.0$]{\includegraphics[width=.3\textwidth]{img/delaunay_T5G1.png}}
	\subfigure[$\delta = 1.0, \gamma = 1.0$]{\includegraphics[width=.3\textwidth]{img/delaunay_T1G1.png}}
	...
	\caption{Die Dreiecksnetze a-c wurden mit dem \textit{FloydSteinberg}-Ansatz erzeugt und die Dreicksnetze d-i mit dem \textit{Quadtree}-Ansatz.}
	\label{fig:netting_fq}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Vergleich von nachbearbeiteten Dreiecksnetzen, der Szene \textit{CoolRandom}, die mit dem \textit{Quadtree}-Ansatz erzeugt wurden. 
			 Dabei wurde $\alpha = 10$ und $\beta = 0.2$ gewählt.}
	\label{fig:nettings}
\end{figure}

Da die Nachbearbeitung für alle Dreiecke unabhängig voneinander berechnet
werden kann, lässt sich dieser Schritt gut parallelisieren.
Auf diese Weise kann der Nachbearbeitungsschritt um ein Vielfaches beschleunigt werden.
...
\begin{figure}[H]
	...
	\subfigure[$t_{max} = 10$]{\includegraphics[width=.3\textwidth]{img/delaunay10_L7I6.png}}
	\subfigure[$t_{max} = 9$]{\includegraphics[width=.3\textwidth]{img/delaunay9_L7I6.png}}
	\subfigure[$t_{max} = 8$]{\includegraphics[width=.3\textwidth]{img/delaunay8_L7I6.png}}
	
	\subfigure[$t_{max} = 10$]{\includegraphics[width=.3\textwidth]{img/frame_00000_D10.png}}
	\subfigure[$t_{max} = 9$]{\includegraphics[width=.3\textwidth]{img/frame_00000_D9.png}}
	\subfigure[$t_{max} = 8$]{\includegraphics[width=.3\textwidth]{img/frame_00000_D8.png}}
	\caption{Beispiele für Schnittkantenartefakte in der Szene \textit{TestSpheres}, des ersten Frames. Das Netz wurde mit $l = 0.7, i = 0.6, \alpha = 100, \beta = 0.2$ erzeugt.}
	\label{fig:refinementArtefakt}
\end{figure}

Aufgrund der Messergebisse vom SSIM und dem PSNR lässt sich sagen, 
...
so dass hier eine gute Approximation bis zu 20 Grad Winkelunterschied möglich ist.
...
entsteht bei größeren Kamerawinkelunterschieden zwangsläufig ein Fehler.
Die Abbildung \ref{fig:beleuchtung_fehler} zeigt ein Beispiel dafür.
Dieser steigt mit der Größe des Kamerawinkelunterschiedes und taucht in den getesteten Datensätzen nur bei sehr großen Winkeln auf
und ist im Verhältnis zu den anderen entstehenden Artefakten jedoch zu vernachlässigen.
...
\begin{figure}[H]
	\centering	
	\subfigure[\textit{Ground-Truth}]{\includegraphics[width=.49\textwidth]{img/testspheres_00189.png}}
	\subfigure[\textit{Vollvernetzung}]{\includegraphics[width=.49\textwidth]{img/frameFULL_00490.png}}
	\caption{Zu sehen ist der Frame 189, aus der dritten Sequenz der Szene \textit{TestSpheres}.
	Der Kamerawinkelunterschied beträgt $80$ Grad. Es tritt neben den geometrischen Artefakten ein zusätzlich Beleuchtungsfehler auf.}
	\label{fig:beleuchtung_fehler}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../results/6/512x512_Delaunay/T0.8/G0.8/delaunay.png}
	\caption{\textit{FloydSteinberg}-Ansatz mit den Parametern $\delta = 0.8, \gamma = 0.8$.}
	\label{fig:floyd_steinberg_nett}
\end{figure}

\chapter{Zusammenfassung und Ausblick}

In Rahmen dieser Arbeit wurde ein Remote-Visualisierungssystem konstruiert.
Der Server erzeugt in diesem, zusätzlich zum Farbbild, ein adaptives Dreiecksnetz
aus dem dazugehörigen Tiefenbild.
Das Farbbild und das Dreiecksnetz werden anschließend zum Client gesendet,
der eine Extrapolation auf Basis der Rückprojektion durchführt.
Mit Hilfe von zwei \textit{Ground-Truth}-Datensätzen wurde
untersucht, wie sich die Güte in Abbhängigkeit des Kamerawinkelunterschiedes
verhält.
Dabei haben die Dreiecksnetze die Aufgabe, die Grauwertverläufe der Tiefenbilder
zu approximieren.
Konstruiert werden die Dreiecksnetze mit Hilfe der Delaunay-Triangulierung.
Die für die Vernetzung notwendige Menge von ausgewählten Punkten
wird anhand von Gradientenbildern bestimmt.
Zu diesem Zweck wurde ein Fehlerdiffusionsverfahren angewendet, 
welches auf dem \textit{FloydSteinberg}-Algorithmus basiert, und ein \textit{Quadtree}-Ansatz
implementiert.
Beide wurden hinsichtlich der Konstruktionszeiten, der Anzahl von Dreiecken und 
der erzielten Güte untersucht.
Dabei hat sich gezeigt, dass der \textit{Quadtree}-Ansatz hinsichtlich aller
Kriterien bessere Ergebnisse hervorbringt.
Weiterhin wurde ein Nachbearbeitungsschritt implementiert, mit dem
die Qualität der Netze hinsichtlich der Güte, insbesondere bei großen Winkeln,
deutlich verbessert wird, allerdings zulasten der Dreiecksanzahl und der Konstruktionszeit.
Dabei treten jedoch bei sehr kleinen Kamerawinkelunterschieden Artefakte in der
Nähe von großen Tiefensprüngen im Tiefenbild auf.

Die Extrapolation, auf Basis der Rückprojektion von Dreicksnetzen, ist durchaus ein geeignetes Verfahren, um neue Ansichten einer Szene durch den Client zu approximieren,
vorausgesetzt, der Kamerawinkelunterschied wird nicht zu groß.
Die Dreiecksnetze, erzeugt mit dem \textit{Quadtree}, lassen sich mit beliebiger
Genauigkeit generieren und sind eine gute Grundlage, um diese verlustbehaftet komprimieren
zu können.

Der Nachbearbeitungschritt führt zu einer deutlichen Verbesserung hinsichtlich
der Güte, lässt sich aber sicherlich noch verbessern;
insbesondere im Bezug auf die Anzahl der erzeugten Dreiecke.
Im nächsten Schritt, könnten die Dreiecksnetze durch Valenz-basierte Kodierung
weiter komprimiert werden.
Weiterhin sollte eine binäre-Datenbeschreibungssprache gewählt werden, um
die Größe der Nachrichtenpakete zu verkleinern.
Durch Verdeckung bedingte Fehler könnten mit Hilfe des Kameramodells von Popescu und Aliaga \cite{Popescu:2006:DDO:1111411.1111436} weiter reduziert werden.
Desweiteren sollte untersucht werden, inwieweit die extrapolierten Bilder
genutzt werden können, um Informationen bei der Übertragung des Farbbildes
mit dem Differenzbildansatz zu reduzieren.
Progressive Übertragung der Dreiecksnetze könnte genutzt werden,
um verschiedenen Detailstufen an den Client zu übertragen. 
Außerdem kann der Client in Abhängigkeit seiner Ressourcen ein eigenes Modell
der Szene, aus den übertragenen Dreiecksnetzen konstruieren.
